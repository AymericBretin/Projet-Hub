...
public class MoveOnMap : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Actual_Level == 1 && Input.GetButtonDown("Jump") && Actual_Level <= Level_Unlock) {
            SceneManager.LoadScene(scenePaths[0], LoadSceneMode.Single);
        }
        if (Actual_Level == 2 && Input.GetButtonDown("Jump") && Actual_Level <= Level_Unlock) {
            SceneManager.LoadScene(scenePaths[1], LoadSceneMode.Single);
        }
        if (Actual_Level == 3 && Input.GetButtonDown("Jump") && Actual_Level <= Level_Unlock) {
            SceneManager.LoadScene(scenePaths[2], LoadSceneMode.Single);
        }
        if (Actual_Level == 4 && Input.GetButtonDown("Jump") && Actual_Level <= Level_Unlock) {
            SceneManager.LoadScene(scenePaths[3], LoadSceneMode.Single);
        }
        if (Actual_Level == 5 && Input.GetButtonDown("Jump") && Actual_Level <= Level_Unlock) {
            SceneManager.LoadScene(scenePaths[4], LoadSceneMode.Single);
        }
        if (Actual_Level == 6 && Input.GetButtonDown("Jump") && Actual_Level <= Level_Unlock) {
